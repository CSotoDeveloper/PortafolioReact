...
export function Navbar() {
  ...
  return (
    <header className='sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 px-4 lg:px-0 overflow-hidden'>
      ...
      {mobileMenuOpen && (
        <div ref={mobileMenuRef} className='sm:hidden    bg-opacity-75 z-40    '>  
                <div className='flex flex-col items-end space-y-1 px-2 pb-3 pt-2 overflow-hidden justify-center'>
                <button type='button' onClick={() => setMobileOpen(!mobileMenuOpen)}>
              <a href="#about" className=' rounded-md px-3 py-2 text-base font-medium text-blue-500 hover:bg-gray-50 hover:text-gray-900 overflow-hidden'>Acerca de mi</a>
            </button>
            <button type='button' onClick={() => setMobileOpen(!mobileMenuOpen)}>
              <a href="#education" className=' rounded-md px-3 py-2 text-base font-medium text-blue-500 hover:bg-gray-50 hover:text-gray-900'>Educación</a>
            </button>
            <button type='button' onClick={() => setMobileOpen(!mobileMenuOpen)}>
              <a href="#experience" className=' rounded-md px-3 py-2 text-base font-medium text-blue-500 hover:bg-gray-50 hover:text-gray-900'>Experiencia</a>
            </button>
            <button type='button' onClick={() => setMobileOpen(!mobileMenuOpen)}>
              <a href="#projects" className=' rounded-md px-3 py-2 text-base font-medium text-blue-500 hover:bg-gray-50 hover:text-gray-900'>Proyectos</a>
            </button>
            <button type='button' onClick={() => setMobileOpen(!mobileMenuOpen)}>
              <a href="#contact" className=' rounded-md px-3 py-2 text-base font-medium text-blue-500 hover:bg-gray-50 hover:text-gray-900'>Contacto</a>
            </button>
